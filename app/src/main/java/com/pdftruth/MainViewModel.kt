...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun renderCurrentPageForScale(scale: Float) {
        ...
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "zoom render requested after debounce: scale=$scale")
        }
        renderPageInternal(
    ...
    private fun renderPageInternal(
        ...
        renderJob = viewModelScope.launch {
            try {
                ...
                if (BuildConfig.DEBUG && renderScale > 1f && !updateRecentDocument) {
                    Log.d(TAG, "zoom render completed: scale=$renderScale")
                }
